...
def AQS_reader(filename: str):
    ...
    state = data["State Name"].to_list()
    data = []
    # return a structure
    return surface_data(lat, lon, date, airtemp, state)

def wrf_reader_core(met_file_2d_file):

        print("Currently reading: " + met_file_2d_file.split('/')[-1])
        # reading time and coordinates
        lat = _read_nc(met_file_2d_file, 'XLAT')
        lon = _read_nc(met_file_2d_file, 'XLONG')
        time_var = _read_nc(met_file_2d_file, 'XTIME')
        time_unit = _get_nc_attr(met_file_2d_file, 'XTIME')
...
def wrf_reader(dir_wrf: str, YYYYMM: str) -> list:
    '''
       MINDS reader
       Inputs:
             product_dir [str]: the folder containing the GMI data
    ...
    # read meteorological and chemical fields
    met_files_2d = sorted(
            glob.glob(dir_wrf + "/wrfout_d01_*" + YYYYMM[0:4] + '-' + YYYYMM[4:]  + "*"))
    #met_files_3d = sorted(glob.glob(dir_mcip + "/METCRO3D_*" + YYYYMM  + "*"))
    # define gas profiles for saving
    outputs = []
    for k in range(len(met_files_2d)):
        outputs.append(wrf_reader_core(met_files_2d[k]))
...
def colocate(ctmdata, airdata, date1, date2):
    ...
    for t1 in range(0, np.size(airdata.time)):
        ...
        closest_index_day = int(np.floor(closest_index/np.shape(ctmdata[0].TEMP2)[0]))
        closest_index_hour = int(closest_index % np.shape(ctmdata[0].TEMP2)[0])
        ...
        airdata_value_mapped.append(airdata.value[t1])
        airdata_time_mapped.append(airdata.time[t1])
        airdata_lat_mapped.append(airdata.lat[t1])
        airdata_lon_mapped.append(airdata.lon[t1])
    ...
    output["AQS_TEMP2"] = np.array(airdata_value_mapped)
    output["time"] = airdata_time_mapped
